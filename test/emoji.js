...
describe("emoji.js", function() {
  ...
  describe("emojify(str)", function() {
    it("should parse :emoji: in a string and replace them with the right emoji", function() {
      var coffee = emoji.emojify(
        ":palms_up_together_medium_skin_tone::thumbs_up_light_skin_tone::OK_hand::cold_face::man_fairy_dark_skin_tone:"
      );
      should.exist(coffee);
      coffee.should.be.exactly("🤲🏽👍🏻👌🥶🧚🏿‍♂");
    });
    ...
    it("should handle flags correctly", function() {
      var flags = emoji.emojify(
        "Mexico :flag_Mexico: and Morocco :flag_Morocco: are not the same"
      );
      should.exists(flags);
      flags.should.be.exactly("Mexico 🇲🇽 and Morocco 🇲🇦 are not the same");
    });
    ...
    it("should wrap emoji using provided format function", function() {
      var coffee = emoji.emojify("I :heart: :coffee:", null, function(
        code,
        name
      ) {
    ...
    it("should not wrap unknown using provided format function", function() {
      var coffee = emoji.emojify("I :unknown_emoji: :coffee:", null, function(
        code,
        name
      ) {
        return '<img alt="' + code + '" src="' + name + '.png" />';
  ...
  describe("replace", function() {
    ...
    it("Should be able to replace by callback function", function() {
      var result = emoji.replace("There is no ⚠ on my hard drive", function(
        emoji
      ) {
        return emoji.key;
      });
